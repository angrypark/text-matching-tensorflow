...
class DualEncoderTransformerDense(Model):
    def __init__(self, dataset, config, mode=tf.contrib.learn.ModeKeys.TRAIN):
        super(DualEncoderTransformerDense, self).__init__(dataset, config)
        if mode == "train":
    ...
    def build_model(self):
        ...
        with tf.variable_scope("transformer"):
            ...
            self.queries_encoded = tf.squeeze(tf.reduce_sum(self.queries_encoded, axis=1, keep_dims=True))
            self.replies_encoded = tf.squeeze(tf.reduce_sum(self.replies_encoded, axis=1, keep_dims=True))
        ...
        with tf.variable_scope("sampling"):
            ...
            self.positive_inputs = tf.concat([self.queries_encoded, self.positive_distances, self.replies_encoded], 1)
