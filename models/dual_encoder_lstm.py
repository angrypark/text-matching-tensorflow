    ...
    def build_model(self):
        ...
        with tf.variable_scope("inputs"):
            ...
            self.input_replies = tf.placeholder_with_default(next_batch["input_replies"],
                                                             [None, self.config.max_length],
                                                             name="input_replies")
            self.query_lengths = tf.placeholder_with_default(next_batch["query_lengths"],
                                                             [None, 1],
                                                             name="query_lengths")
            self.reply_lengths = tf.placeholder_with_default(next_batch["reply_lengths"],
                                                             [None, 1],
                                                             name="reply_lengths")
        ...
        with tf.variable_scope("lstm_layer") as vs:
            ...
            _, replies_encoded = tf.nn.dynamic_rnn(
                ...
                sequence_length=tf.cast(tf.squeeze(self.reply_lengths), tf.float32),
                dtype=tf.float32,
            )
        ...
        with tf.variable_scope("sampling"):
            ...
            self.echo_distances = tf.matmul(self.queries_encoded, self.echo_encoded)
        ...
        with tf.variable_scope("prediction"):
            self.positive_logits = tf.gather(tf.reshape(self.distances, [-1]), tf.where(positive_mask), 1)
            self.negative_logits = tf.gather(tf.reshape(self.distances, [-1]), tf.where(negative_mask), 1)
            self.echo_logits = tf.gather(tf.reshape(self.echo_distances, [-1]), tf.where(positive_mask), 1)
            ...
            echo_mask = tf.cond(self.add_echo, 
                                lambda: tf.ones_like(self.logits), 
                                lambda: tf.concat([tf.ones_like(tf.concat([self.positive_logits,
                                                                           self.negative_logits], axis=0)), 
                                                   tf.zeros_like(self.echo_logits)], axis=0))
            self.logits = tf.gather(self.logits, tf.where(echo_mask), 1)
            self.labels = tf.gather(self.labels, tf.where(echo_mask), 1)
